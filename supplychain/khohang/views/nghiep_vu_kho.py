...
def auto_create_order(request):
    if request.method == 'POST':
        try:
            ...
            last_order_item = DonDatHang_CT.objects.filter(sanPham=san_pham).order_by(
                '-donDatHang__ngayDatHang').first()
            ...
            return JsonResponse({'status': 'success',
                                 'message': f'Đã thêm {san_pham.tenSP} (SL: {qty_to_order}) vào đơn hàng tự động {don_dat_hang.maDatHang}.'})
...
def nhapkho(request):
    if request.method == 'POST':
        try:
            ...
            if not items:
                return JsonResponse({'status': 'error', 'message': 'Phiếu nhập phải có ít nhất 1 sản phẩm.'},
                                    status=400)
            ...
            if not created:
                ...
                if old_trangthai in [1, -1]:
                    return JsonResponse(
                        {'status': 'error', 'message': 'Không thể sửa phiếu đã hoàn thành hoặc đã hủy.'}, status=400)
    ...
    # GET request
    inbound_records = NhapKho.objects.select_related('nhaCungCap').prefetch_related(
        'phieunhap_ct_set__sanPham').order_by('-ngayNhap')
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and 'maPhieuNhap' in request.GET:
        ...
        status_class_map = {0: 'bg-yellow-100 text-yellow-700', 1: 'bg-green-100 text-green-700',
    ...
    pos = DonDatHang.objects.filter(trangThai=2).select_related('nhaCungCap').prefetch_related(
        'dondathang_ct_set__sanPham')
    po_list = []
    for po in pos:
        po_list.append({
            'code': po.maDatHang,
...
def trahang(request):
    if request.method == 'POST':
        try:
            ...
            if import_record.trangthaiNhap != 1:
                return JsonResponse(
                    {'status': 'error', 'message': 'Chỉ có thể tạo phiếu trả hàng cho phiếu nhập đã hoàn thành.'},
                    status=400)
        ...
        try:
            ...
            if tra_hang.trangThai in [1, -1]:
                return JsonResponse({'status': 'error', 'message': 'Không thể sửa phiếu đã hoàn thành hoặc đã hủy.'},
                                    status=400)
        ...
        try:
            ...
            if tra_hang.trangThai == -1:
                ...
                return JsonResponse({'status': 'error', 'message': 'Không thể xóa phiếu trả hàng đã hoàn thành.'},
                                    status=400)
        except Exception as e:
            return JsonResponse({'status': 'error', 'message': str(e)}, status=400)

    # Xu ly lay chi tiet phieu tra qua AJAX
    elif request.method == 'GET' and request.headers.get(
            'x-requested-with') == 'XMLHttpRequest' and 'maPhieuTra' in request.GET:
...
def xuatkho(request):
    if request.method == 'POST':
        try:
            ...
            if not items and trangThai != -1:
                return JsonResponse({'status': 'error', 'message': 'Phiếu xuất phải có ít nhất 1 sản phẩm.'},
                                    status=400)
        ...
        try:
            ...
            if export_record.trangThai == -1:
                ...
                return JsonResponse({'status': 'error', 'message': 'Không thể xóa phiếu xuất đã hoàn thành.'},
                                    status=400)
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and 'maPhieuXuat' in request.GET:
        ...
        status_class_map = {0: 'bg-yellow-100 text-yellow-700 border-yellow-200',
...
def kiemke(request):
    if request.method == 'POST':
        try:
            ...
            if not items:
                return JsonResponse({'status': 'error', 'message': 'Phiếu kiểm kê phải có ít nhất 1 sản phẩm.'},
                                    status=400)
        ...
        try:
            ...
            if check_record.trangThai == -1:
                ...
                return JsonResponse({'status': 'error', 'message': 'Không thể xóa phiếu kiểm kê đã cân bằng.'},
                                    status=400)
